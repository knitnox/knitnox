...
\r
    <style>\r
      html, body { margin: 0; padding: 0; width: 100%; height: 100%; }\r
      #app { width: 100%; min-height: 100vh; min-height: 100dvh; display: flex; flex-direction: column; }\r
      #splash {\r
        position: fixed; inset: 0; background: #e6e9ef;\r
        display: flex; align-items: center; justify-content: center;\r
        z-index: 9999; transition: opacity 0.45s ease, visibility 0.45s ease;\r
      }\r
      #splash.hidden { opacity: 0; visibility: hidden; }\r
      .splash-card {\r
        display: flex; flex-direction: column; align-items: center; gap: 18px;\r
        padding: 40px 48px; border-radius: 28px; background: #e6e9ef;\r
        box-shadow: 10px 10px 20px #c2c8d0, -10px -10px 20px #ffffff;\r
        animation: splash-pop 0.4s cubic-bezier(0.34,1.56,0.64,1) both;\r
      }\r
      @keyframes splash-pop { from { transform: scale(0.85); opacity: 0; } to { transform: scale(1); opacity: 1; } }\r
      .splash-icon { width: 88px; height: 88px; }\r
      .splash-name { font-family: 'Orbitron', sans-serif; font-size: 1.5rem; font-weight: 700; color: #2a2f3a; margin: 0; letter-spacing: 0.04em; }\r
      .splash-brand { font-size: 0.8rem; font-weight: 600; color: #8a9ab0; margin: 0; letter-spacing: 0.18em; text-transform: uppercase; }\r
    </style>\r
  </head>\r
  <body>\r
